 ...
 *
 * Removes all tracking data and wipes the database.
...
function main(): void {
    console.log('\n');
    console.log('========================================');
    console.log('  Carbonlog Uninstall                   ');
    console.log('========================================');
    console.log('\n');
    ...
    // Also clean up legacy database file if it exists (pre-rename: carbon-tracker.db)
    const legacyDbPath = getLegacyDatabasePath();
    if (fs.existsSync(legacyDbPath)) {
        fs.unlinkSync(legacyDbPath);
        for (const suffix of ['-wal', '-shm']) {
            const walPath = legacyDbPath + suffix;
            if (fs.existsSync(walPath)) {
                fs.unlinkSync(walPath);
            }
        }
        console.log(`  Deleted legacy database: ${legacyDbPath}`);
    }
