...
describe('isPluginStatusLine', () => {
    it('detects statusline-carbonlog', () => {
        expect(isPluginStatusLine('node /path/to/statusline-carbonlog.js')).toBe(true);
    });
...
describe('getInstalledPluginEntries', () => {
    ...
    it('returns entries for the carbon plugin', () => {
        ...
        fs.writeFileSync(
            path.join(tmpDir, 'plugins', 'installed_plugins.json'),
            JSON.stringify({ version: 2, plugins: { 'carbonlog@cnaught-plugins': entries } })
        );
    ...
    it('cleans up project settings and converts to user scope', () => {
        ...
        fs.writeFileSync(
            ...
            JSON.stringify({
                ...
                enabledPlugins: { 'carbonlog@cnaught-plugins': true }
            })
        );
        fs.writeFileSync(
            ...
            JSON.stringify({
                ...
                enabledPlugins: { 'carbonlog@cnaught-plugins': true, 'other-plugin': true }
            })
        );
        ...
        expect(globalSettings.enabledPlugins['carbonlog@cnaught-plugins']).toBe(true);
        ...
        const entries = installed.plugins['carbonlog@cnaught-plugins'];
...
describe('cleanupAllInstallations', () => {
    ...
    it('cleans up all settings files and installed_plugins.json', () => {
        ...
        expect(installed.plugins['carbonlog@cnaught-plugins']).toBeUndefined();
...
describe('configureSettings', () => {
    ...
    it('installs standalone when existing statusline is carbon', () => {
        fs.writeFileSync(
            targetSettingsPath,
            JSON.stringify({
                statusLine: { type: 'command', command: 'node /old/path/carbonlog-statusline.ts' }
            })
        );
    ...
    it('preserves existing settings keys', () => {
        ...
        expect(settings.enabledPlugins).toEqual({ 'carbonlog@cnaught-plugins': true });
        ...
        fs.writeFileSync(
            ...
            JSON.stringify({
                statusLine: {
                    type: 'command',
                    command:
                        'npx -y bun /old/cache/carbon/2.4.0/src/statusline/carbonlog-statusline.ts'
                }
            })
        );
...
// because they import from data-store which conflicts with the mock in carbonlog-output.test.ts.
