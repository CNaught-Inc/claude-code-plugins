...
const PLUGIN_ID = 'carbonlog@cnaught-plugins';
...
export function isPluginStatusLine(command: string): boolean {
    return (
        command.includes('statusline-carbonlog') ||
...
/**
 * Read all carbonlog plugin entries from ~/.claude/plugins/installed_plugins.json.
 */
...
/**
 * Remove the carbonlog statusline from a settings file.
 * If _carbonlogOriginalStatusLine is present, restores it as the statusLine.
 * Also removes carbonlog from enabledPlugins.
 */
function cleanupSettingsFile(settingsPath: string): void {
    try {
        ...
        // Restore original statusline or remove carbonlog statusline
        const statusLine = settings.statusLine as Record<string, unknown> | undefined;
        const savedOriginal = settings._carbonlogOriginalStatusLine as
 ...
 *
 * - Cleans up carbonlog statusline + enabledPlugins from all project/local settings files
 * - Ensures enabledPlugins in ~/.claude/settings.json has the carbonlog plugin
...
/**
 * Clean up all carbonlog plugin traces during uninstall.
 *
...
/**
 * Update the carbonlog statusline command in settings.json if the plugin root has changed.
 *
 ...
 *
 * If an existing non-carbonlog statusline is found, it is preserved and wrapped
 * so both statuslines run together (original output | carbonlog output).
 */
