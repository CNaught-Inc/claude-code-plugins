 ...
 * Database location: ~/.claude/carbonlog.db (prod)
 *                    ~/.claude/carbonlog-<env>.db (non-prod)
 */
...
export function getDatabasePath(): string {
    return path.join(getClaudeDir(), `carbonlog${getDbEnvSuffix()}.db`);
}

/**
 * Get the legacy database file path (pre-rename: carbon-tracker.db).
 * Used by migration to find and rename old database files.
 */
