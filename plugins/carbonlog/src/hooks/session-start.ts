 ...
 * Session Start Hook
 *
 * Initializes the Carbonlog tracker when a Claude Code session starts.
 * - Migrates from legacy "carbon" name if needed
...
async function main(): Promise<void> {
    try {
        ...
        // Migrate from legacy "carbon" plugin name if needed (before any DB access)
        migrateFromCarbon(getClaudeDir());
