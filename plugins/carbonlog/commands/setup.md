...
Set up the CNaught Carbonlog plugin.
...
```bash
npx -y bun ${CLAUDE_PLUGIN_ROOT}/src/scripts/carbonlog-setup-check.ts
```
...
```bash
npx -y bun ${CLAUDE_PLUGIN_ROOT}/src/scripts/carbonlog-setup.ts --team "Team Name" [--backfill] [--disable-sync]
```
