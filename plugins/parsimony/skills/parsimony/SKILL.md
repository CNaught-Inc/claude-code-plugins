...
Track savings mentally. **High-value** (prioritize these): line-range read ~(skipped_lines)*10 tokens, eliminated tool call ~500-1k, avoided re-read ~file size. **Low-value**: skipped preamble ~50-100.

## `/token-report`

```
Mode: [mode] | Saved: ~[total] tokens
