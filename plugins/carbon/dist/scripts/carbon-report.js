...
function calculateEquivalents(co2Grams) {
  return {
...
function getProjectStats(db, days = 7) {
  const stmt = db.prepare(`
        SELECT
            project_path,
            COUNT(*) as sessions,
...
// src/scripts/carbon-report.ts
function formatNumber(num) {
  return num.toLocaleString();
}
function generateBar(value, maxValue, maxWidth = 20) {
  ...
  try {
    ...
    if (projectStats.length > 0) {
      ...
      for (const project of topProjects) {
        ...
        const percent = totalProjectCO2 > 0 ? `(${Math.round(project.co2Grams / totalProjectCO2 * 100)}%)` : "";
        console.log(`  ${name} ${co2Str} ${percent}`);
      }
      if (projectStats.length > 5) {
        const otherCO2 = projectStats.slice(5).reduce((sum, p) => sum + p.co2Grams, 0);
        const otherPercent = totalProjectCO2 > 0 ? `(${Math.round(otherCO2 / totalProjectCO2 * 100)}%)` : "";
        console.log(
...
//# sourceMappingURL=carbon-report.js.map
