...
function saveAuthConfig(db, config) {
...
async function fetchDiscovery(auth0Domain2) {
  const url = `https://${auth0Domain2}/.well-known/openid-configuration`;
...
async function exchangeCodeForTokens(tokenEndpoint, code, clientId, codeVerifier) {
...
async function main() {
  ...
  try {
    ...
    const authConfig = {
      ...
      accessTokenExpiresAt: new Date(Date.now() + result.expiresIn * 1e3),
      refreshTokenExpiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3),
      organizationId: null,
...
//# sourceMappingURL=browser-oauth.js.map
