...
var StatuslineInputSchema = external_exports.object({
  ...
  context_window: external_exports.object({
    current_usage: external_exports.object({
      input_tokens: external_exports.number().optional(),
      output_tokens: external_exports.number().optional(),
      cache_creation_input_tokens: external_exports.number().optional(),
      cache_read_input_tokens: external_exports.number().optional()
...
function isCarbonStatusLine(command) {
  return command.includes("statusline-carbon") || command.includes("carbon-statusline");
}
function configureSettings() {
  const projectDir = process.cwd();
  const claudeProjectDir = path3.join(projectDir, ".claude");
  const settingsPath = path3.join(claudeProjectDir, "settings.local.json");
  const pluginRoot = getPluginRoot();
  const bunRunner = path3.join(pluginRoot, "scripts", "bun-runner.js");
  const standaloneScript = path3.join(pluginRoot, "dist", "statusline", "carbon-statusline.js");
  const wrapperScript = path3.join(pluginRoot, "dist", "statusline", "statusline-wrapper.js");
  try {
    let settings = {};
    ...
    if (!existingStatusLine || typeof existingStatusLine !== "object") {
      const globalSettingsPath = path3.join(getClaudeDir(), "settings.json");
      try {
        if (fs3.existsSync(globalSettingsPath)) {
          const globalContent = fs3.readFileSync(globalSettingsPath, "utf-8");
          const globalSettings = JSON.parse(globalContent);
          if (globalSettings.statusLine && typeof globalSettings.statusLine === "object") {
            existingStatusLine = globalSettings.statusLine;
          }
        }
      } catch {
      }
    }
    const hasExternalStatusLine = existingStatusLine && typeof existingStatusLine === "object" && typeof existingStatusLine.command === "string" && !isCarbonStatusLine(existingStatusLine.command);
    if (hasExternalStatusLine) {
      settings._carbonOriginalStatusLine = { ...existingStatusLine };
      const originalCommand = existingStatusLine.command;
      settings.statusLine = {
        type: "command",
    ...
    if (!fs3.existsSync(claudeProjectDir)) {
      fs3.mkdirSync(claudeProjectDir, { recursive: true });
    }
...
function migrateFromGlobalStatusline() {
  const claudeDir = getClaudeDir();
  const globalSettingsPath = path3.join(claudeDir, "settings.json");
  try {
    if (fs3.existsSync(globalSettingsPath)) {
      const content = fs3.readFileSync(globalSettingsPath, "utf-8");
      const settings = JSON.parse(content);
      const statusLine = settings.statusLine;
      if (statusLine && typeof statusLine === "object" && typeof statusLine.command === "string" && (statusLine.command.includes("statusline-carbon") || statusLine.command.includes("carbon-statusline"))) {
        delete settings.statusLine;
        fs3.writeFileSync(globalSettingsPath, JSON.stringify(settings, null, 2));
        console.log("  Removed old global statusline config from ~/.claude/settings.json");
      }
    }
  } catch {
  }
  const wrapperPath = path3.join(claudeDir, "statusline-carbon.mjs");
  try {
    if (fs3.existsSync(wrapperPath)) {
      fs3.unlinkSync(wrapperPath);
...
//# sourceMappingURL=carbon-setup.js.map
