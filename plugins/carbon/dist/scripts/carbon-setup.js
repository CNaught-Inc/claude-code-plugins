...
function installStatusline() {
  ...
  const statuslinePath = path2.join(claudeDir, "statusline-carbon.mjs");
  try {
    if (!fs2.existsSync(claudeDir)) {
      fs2.mkdirSync(claudeDir, { recursive: true });
    }
    fs2.writeFileSync(statuslinePath, STATUSLINE_SCRIPT, { mode: 493 });
    return {
      success: true,
      path: statuslinePath,
...
function configureSettings() {
  ...
  try {
    ...
    if (existingStatusLine && typeof existingStatusLine === "object" && existingStatusLine.command !== statuslinePath) {
      return {
        success: false,
...
`);
    ...
    if (authConfig) {
      const isExpired = authConfig.refreshTokenExpiresAt < /* @__PURE__ */ new Date();
      console.log(
